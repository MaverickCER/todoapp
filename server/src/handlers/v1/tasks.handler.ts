...
async function handleUpdate(where: { id: number; profileId: number }, data: Partial<TTask>): Promise<TTask> {
  return await taskRepository.update(where, data);
}

async function handleRemove(where: { id: number; profileId: number }): Promise<GetBatchResult> {
  if (where.id === 0) return await taskRepository.delete({ profileId: where.profileId });
