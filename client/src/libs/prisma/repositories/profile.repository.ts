...
import { profileSchemaCreate, profileSchemaUniqueWhere, profileSchemaUpdate, TProfile } from '@/libs/zod';
...
export const profileRepository = {
   ...
   * @throws {Error} Validation errors if the arguements are invalid.
   */
  async update(where: { id: number } | { email: string }, data: TProfile) {
   ...
   * @throws {Error} Validation errors if the arguements are invalid.
   */
  async delete(where: { id: number } | { email: string }) {
    const validatedWhere = profileSchemaUniqueWhere.safeParse(where);
    if (!validatedWhere.success) throw new Error(validatedWhere.error.message);
   ...
   * @throws {Error} Validation errors if the arguements are invalid.
   */
  async obliterate(where: { id: number } | { email: string }) {
    const validatedWhere = profileSchemaUniqueWhere.safeParse(where);
    if (!validatedWhere.success) throw new Error(validatedWhere.error.message);
